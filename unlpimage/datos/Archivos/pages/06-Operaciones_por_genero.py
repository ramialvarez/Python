...
def operaciones_por_genero(logs,perfiles):
    #Intercambio filas y columnas
    perfiles = perfiles.transpose()

    #Me quedo solo con las filas de las operaciones que me interesan 
    logs = logs[(logs['Operación'] == 'Modificación de imagen previamente clasificada') | (logs['Operación'] == "Nueva imagen clasificada")]
    ...
    colors = plt.get_cmap('Accent')(np.linspace(0.2, 0.7, len(fusion)))
...
titulo_markdown = '<h1 style="font-family: Arial; text-align: center; color: #bf178a;">Uso de la aplicación por género</h1>'
...
subtitulo_analisis_markdown = '<h3 style="font-family: Arial; text-align: left; color: #bf17bc;">Pasos del análisis:</h3>'

# Defino los pasos llevamos a cabo para el análisis de los datos del archivo de logs
pasos_analisis = [
    ('Transformación de los datos', 'Transformo el archivo de logs y el archivo de perfiles en un DataFrame. En el archivo de logs, me quedo solo con las filas en las que la columna "Operación" sea igual a "Modificación de imagen previamente clasificada" o "Nueva imagen clasificada".  En el archivo de perfiles intercambio las filas por las columnas.'),
    ('Contar valores', 'Fusiono ambos DataFrame en uno solo, con la columna de "Nick" como eje de fusión. Una vez que obtengo el DataFrame fusionado, en cada operación que aparece en los logs me va a aparecer el género de quien la realizó. A partir de esto, cuento los valores de la columna "autogenero", que contiene los géneros de las personas que realizaron estas operaciones, utilizando valuecounts.'),
...
for i, (paso, descripcion) in enumerate(pasos_analisis):
    paso_html = f'<h6 style="font-family: Arial; text-align: left; color: #bf178a;">{i+1}. {paso}</h6>'
...
subtitulo_grafico_markdown = '<h3 style="font-family: Arial; text-align: center; color: #bf17bc;">Genero de las personas que realizaron las operaciones de clasificar o modificar imagenes</h3>'
...
c2.pyplot(operaciones_por_genero(logs,perfiles),clear_figure=True)