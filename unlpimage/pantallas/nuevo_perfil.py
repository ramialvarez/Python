import json
import os
from PIL import Image, ImageTk
import PySimpleGUI as sg
from unlpimage.config import logs
from unlpimage.config.rutas import RUTA_IMAGENES_USUARIOS, RUTA_JSON
from unlpimage.pantallas import inicio, menu_principal
import base64
import io

#Contantes
TAMANIO_TEXTO = 16
TAMANIO_TITULO  = 25
green_pill64 = ''
button64 = ''

def image_file_to_bytes(image64, size):
    image_file = io.BytesIO(base64.b64decode(image64))
    img = Image.open(image_file)
    img.thumbnail(size, Image.ANTIALIAS)
    bio = io.BytesIO()
    img.save(bio, format='PNG')
    imgbytes = bio.getvalue()
    return imgbytes

def chequear_existe(nick) :
    """Se le envia un nombre por parametro y devuelve verdadero si el nombre no existe y falso
      en caso contrario
    Args : 
        nick = es el nick que debe buscar para si se encuentra o no.
    Return :
        boolean : False si el nick existe, true sino existe. """
    
    try:
        with open(RUTA_JSON, 'r') as archivo_json:
            archivo = json.load(archivo_json)
    except FileNotFoundError :
        return True
    else :
        if (nick in archivo.keys()) :
            return False
        else :
            return True

def cargar_imagen(ruta_origen) :
    """Descarga la imagen ingresada por el usuario y la retorna
    Args : 
        ruta_origen = valor ingresado en la clave ruta_avatar (es una direccion de una imagen).
    Returns :
        imagen2 = imagen editada para mostrar en pantalla."""
    
    nombre_archivo = os.path.basename(ruta_origen)
    ruta_destino = os.path.join(RUTA_IMAGENES_USUARIOS, nombre_archivo)
    # Leer el archivo de la imagen y escribirlo en un archivo nuevo en la carpeta de destino
    with open(ruta_origen, 'rb') as archivo_origen, open(ruta_destino, 'wb') as archivo_destino:
        archivo_destino.write(archivo_origen.read())
        imagen = Image.open(ruta_origen)
        tamanio = imagen.size
        if(tamanio>(200,2000)):
            imagen = imagen.resize((200, 200))
            imagen.save(ruta_destino) 
        imagen2 = ImageTk.PhotoImage(imagen)
    return imagen2

def controlar_valores (values,current_window) :
    """Controla que todos los campos esten correctamente completos.
    
    Args :
        values = valores del diccionario ingresados por el usuario.
        current_window = ventana actual 
    Returns :
        ok = boolean, retorna verdadero si todos los valores son correctos, en caso contario falso."""
    
    ok = True

    errores = 'Por favor: '

    #Evaluamos nick este completo con caracteres
    nick = values["Nick"]
    if not nick :
        errores += '\n - Complete el campo del nick'              
        ok = False
    else:
        if (chequear_existe(nick) == False) :
            errores += '\n - Ingrese otro nick, el ingresado ya existe'
    
    #Evaluamos nombre este completo con caracteres
    nombre = values["Nombre"]
    if not nombre :
        errores += '\n - Complete le campo del nombre'
        ok = False 
           
    #Evaluamos que la edad sea un numero entero y este en un rango
    edad = values["Edad"]
    if  edad.isdigit() :
        edad = int(edad)
        if not 0<= edad <= 150 :
            errores += '\n - Completar el campo de la edad con un numero entero entre 0 y 150'
            ok = False
    else :
        errores += '\n - Completar el campo de la edad con un numero entero entre 0 y 150'
        ok = False   
    
    #Evaluamos que opcion de genero selecciono
    genero = (values["F"], values["M"], values["O"])
    match genero:
        case False, False, False :
            errores += '\n - Completar correctamente el campo del genero'
            ok = False
        case True, False, False :
            genero = 'Femenino'
            values["autogenero"] = genero
        case False, True, False :
            genero = 'Masculino'
            values["autogenero"] = genero
        case False, False, True :
            otro_genero = values["autogenero"]
            if not otro_genero :
                errores += '\n - Complete su genero autopercibido'
                current_window["texto_genero"].update(visible=True)
                current_window["autogenero"].update(visible=True)
                ok = False
    
    if (ok == False) :
        sg.popup (errores, title='-NUEVO_PERFIL-ERROR-', background_color='brown')
    
    return ok

def carga_json(values):
    """Carga los datos ingresados en un archivo Json, en caso de no exisitir tal archivo, lo 
    crea
    Args : 
        values = diccionario con los valores ingresados"""
    
    try:
        with open(RUTA_JSON, 'r') as archivo_json:
            lista_diccionarios = json.load(archivo_json)
    except FileNotFoundError:
        lista_diccionarios = {}
    finally :
        if (values["-NUEVO_PERFIL-RUTA_AVATAR-"] != '') :
            lista= values["-NUEVO_PERFIL-RUTA_AVATAR-"].replace(os.path.sep,'/').split("/")
            values["-NUEVO_PERFIL-RUTA_AVATAR-"]= lista.pop()
        else :
            values["-NUEVO_PERFIL-RUTA_AVATAR-"] = 'Imagen_defecto.png'
        lista_diccionarios[values["Nick"]] = values
        with open(RUTA_JSON, 'w') as archivo_json:
            json.dump(lista_diccionarios, archivo_json, indent=4)

def layout() :
    """Crea elementos para la ventana de nuevo perfil.
    Returns  :
        lista con elementos de la ventana."""

    encabezado = [
        [
            sg.Column(
            [[sg.RButton('Volver', image_data=image_file_to_bytes(button64, (100, 50)), 
             font='Rockwell', pad=(0, 0), key='-NUEVO_PERFIL-VOLVER-')]]
            ),
            sg.Text("Nuevo Perfil", font=('Rockwell', TAMANIO_TITULO), justification="center", pad = (290,1))
        ]
    ]

    columna1 = [
        [sg.Text("Nick", background_color='brown')],
        [sg.InputText(key="Nick")],
        [sg.Text("Nombre", background_color='brown')],
        [sg.InputText(key="Nombre")],
        [sg.Text("Edad", background_color='brown')], 
        [sg.InputText(key="Edad")],
        [sg.Text("Genero",background_color='brown')], 
        [sg.Radio("Femenino", "RADIO1", key = "F",background_color='pink'), 
         sg.Radio("Masculino", "RADIO1", key = "M", background_color='blue'), 
         sg.Radio("Otro", "RADIO1", key = "O", background_color='green')],
        [sg.Text("Ingrese su genero autopercibido", key='texto_genero', background_color='brown', visible=False)],
        [sg.InputText(key='autogenero', visible=False)]
    ]

    columna2 = [
        [sg.Image(
            source= os.path.join(RUTA_IMAGENES_USUARIOS, 'Imagen_defecto.png'),
            key="imagen_mostrar",
            size=(300,300),
            pad=((125,125),(10,10))
        )],
        [sg.FileBrowse(
            "Seleccionar imagen",
            key="-NUEVO_PERFIL-RUTA_AVATAR-",
            enable_events=True,
            change_submits=True,
            size=(20,2),
            pad=(200,1),
            file_types=(('PNG Files', '*.png'),('GIF Files', '*.gif')))
        ]
    ]

    columnas = [
        [
            sg.Column(columna1),
            sg.Column(
                columna2,
                element_justification="left",
                size=(450, 400)
            )
        ]
    ]

    columna_boton = [
        [sg.RButton("Guardar", image_data=image_file_to_bytes(green_pill64, (100, 50)),
         font='Rockwell', pad=(0, 0), key='-NUEVO_PERFIL-GUARDAR-')]
    ]

    return [
        [sg.Column(layout=encabezado, expand_x=True)],
        [
            sg.Column(
                layout=columnas, 
                justification="center", 
                key="llave_columnas",
                expand_y=True, #Expande el tamaño de la columna para que la ocupe entera
                pad=(50, 50) #Deja espacio entre las columnas y con el titulo
            )
        ],
        [sg.Column(layout=columna_boton, expand_x=True, element_justification="right")]
    ]   

def crear_ventana (inicio) :
    """Crea ventana del nuevo perfil
    Args :
        inicio (PySimpleGUI.PySimpleGUI.Window) = pantalla del inicio para luego ser desencondida.
    Returns :
         window (PySimpleGUI.PySimpleGUI.Window) = ventana de nuevo perfil."""
    
    return sg.Window("Nuevo Perfil", layout(), margins=(10,10), background_color='white',metadata= {"inicio":inicio},finalize=True,)

def procesar_eventos_nuevo_perfil (current_window, event, values) :
    """Procesa los eventos del nuevo perfil
    Args: 
        current_window (PySimpleGUI.PySimpleGUI.Window): Ventana de nuevo perfil.
        event (str): Nombre de el evento producido en la ventana.
        values (dict): Diccionario con los valores de la ventana."""
    
    match event :

        case 'RUTA_AVATAR' :
            imagen = cargar_imagen(values["-NUEVO_PERFIL-RUTA_AVATAR-"])
            current_window ["imagen_mostrar"].update(data=imagen, size=(300,300), subsample=3)

        case 'VOLVER' :
            #Vuelve a la pantalla de inicio
            current_window.metadata['inicio'].un_hide()

        case 'GUARDAR' :
                ok = controlar_valores(values,current_window)
                #Si los valores estan completos, carga el archivo json y abre el menu principal
                if ok:
                    carga_json(values)
                    
                    #Actualizar logs
                    logs.actualizar_logs(values["Nick"],'Creación de nuevo perfil')

                    #Abre menu principal
                    menu_principal.crear_ventana(values)
                    
                    #Esconder ventana del nuevo perfil
                    current_window.hide()


        case _ :
            controlar_valores(values, current_window)

